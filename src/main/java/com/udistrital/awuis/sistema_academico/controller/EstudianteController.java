...
public class EstudianteController {
    ...
    public String estudiante(@SessionAttribute(value = "usuario", required = false) Usuario usuario, Model model) {
        ...
        try {
            // Obtener todos los estudiantes y buscar el que coincida
            var estudiantes = estudianteMapper.listarEstudiantes();
            for (Estudiante est : estudiantes) {
                if (est.getIdUsuario() != null && est.getIdUsuario().equals(usuario.getIdUsuario())) {
